...
# @Last Modified time: 2014-03-27 21:50:22
# a script
...
def send_simple_message(email,name,msg, tag = None):
...
if __name__ == '__main__':
	...
	for acc in Account.objects:
		if acc.subscribed:
			...
			if "solo" in acc.username.lower() and plan is not None and len(plan)>0:
				recipe_list = [get_recipe_name_by_id(re) for re in plan]
				temp = template(recipe_list,acc.username)
				email = acc.email
				username = acc.username
				send_simple_message(email,username,temp)
				logging.info("sent to %s"%username)
